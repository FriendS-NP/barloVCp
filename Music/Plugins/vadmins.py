...
async def cbmenu(_, query: CallbackQuery):
    ...
    if not a.can_manage_voice_chats:
        return await query.answer(
            "💡 يمكن للمسؤولين الذين لديهم أذونات إدارة الدردشة الصوتية فقط النقر فوق هذا الزر!",
            show_alert=True,
        )
...
async def skip(client, m: Message):
    ...
    if len(m.command) < 2:
        ...
        if op == 0:
            await m.reply("لا شيء يعمل الان ❌")
        elif op == 1:
            await m.reply(
...
async def cbpause(_, query: CallbackQuery):
    ...
    if chat_id in QUEUE:
        ...
        await query.answer("❌ لا شيء يعمل الان", show_alert=True)
